...
def merge_session_messages_append_only(sidecar_messages: list, state_messages: list) -> list:
    ...
    for msg in sidecar_messages:
        ...
        seen_visible_keys.add(_session_message_visible_key(msg))
        merged_messages.append(msg)
    for msg in state_messages:
        timestamp = _message_timestamp_as_float(msg)
        key = _session_message_merge_key(msg)
        visible_key = _session_message_visible_key(msg)
        if _session_message_content_key(msg) in seen_content_keys:
            continue
        if _has_visible_duplicate(visible_key, seen_visible_keys):
            continue
