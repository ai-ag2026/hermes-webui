...
class Session:
    ...
    def load(cls, sid):
        ...
        # Keep OUR fast orjson-preferring parser (big-session hotpath) rather than
        # upstream's stdlib json.loads; the TOCTOU guard above is orthogonal to it.
        data = _json_loads_session(p.read_text(encoding='utf-8'))
        data['messages'], _collapsed_partials = _collapse_adjacent_duplicate_partials(data.get('messages'))
        session = cls(**data)
    ...
    def compact(self, include_runtime=False, active_stream_ids=None) -> dict:
        ...
        return {
            ...
            **({
            ...
            # Sidecar draft wins over the in-file field (big-session hotpath,
            # 2026-07-13): draft autosaves no longer rewrite the session JSON,
