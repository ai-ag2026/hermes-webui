...
// Tab switching itself lives in switchSkillsPanelTab() (the shared
// .workspace-panel-tab bar for the whole Skills panel: Skills/Toolsets/Hub);
// this section only holds the Hub view's own state and data loading.
...
function switchSkillsPanelTab(tab){
  const target = (tab === 'toolsets' || tab === 'hub') ? tab : 'skills';
  const skillsTab=$('skillsPanelTabSkills'), toolsetsTab=$('skillsPanelTabToolsets'), hubTab=$('skillsPanelTabHub');
  const skillsContent=$('skillsPanelTabContentSkills'), toolsetsContent=$('skillsPanelTabContentToolsets'), hubContent=$('skillsPanelTabContentHub');
  ...
  if(hubTab){hubTab.classList.toggle('active',target==='hub');hubTab.setAttribute('aria-selected',target==='hub'?'true':'false');}
  if(skillsContent) skillsContent.hidden = target!=='skills';
  if(toolsetsContent) toolsetsContent.hidden = target!=='toolsets';
  if(hubContent) hubContent.hidden = target!=='hub';
  ...
  // Skills Hub polls its background action status (install/update/uninstall)
  // while its tab is visible; stop the poll the moment the user tabs away
  // so it doesn't keep firing against a hidden view.
  if(target!=='hub' && _skillsHubPollTimer){ clearInterval(_skillsHubPollTimer); _skillsHubPollTimer=null; }
  if(target==='hub' && !_skillsHubLoadedOnce){
...
async function testMcpServer(name,btnEl){
  ...
  try{
    const r=await api('/api/mcp/servers/'+encodeURIComponent(name)+'/test',{timeoutMs:20000});
    if(resultEl){
      if(r&&r.ok){
        const parts=[];
...
switchSettingsSection=function(name, opts){
  ...
  if(name==='system'){loadMcpServers();loadMcpTools();loadGatewayStatus();loadOpsActionsStatus();loadConfigEditor();}
